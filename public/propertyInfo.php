...
// use id to execute SELECT SQL statements
// Image -- inner join with img table
// retrieve
  // Address
  // price
  // filters
  // num of bedrooms, bathrooms etc

$sql = "SELECT adv.ADVERTISEMENT_ID, ADDRESS_NUMBER, ADDRESS_STREET, ADDRESS_CITY, ADDRESS_STATE, ADVERTISEMENT_PRICE, IMAGE_BINARY
...
if (sqlsrv_execute($stmt) === false) {
  ...
  <div class="container p-5">
    ...
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
      //echo $row['ADDRESS_NUMBER'] . ', ' . $row['ADDRESS_STREET'] . ', ' . $row['ADDRESS_CITY'] . ', ' . $row['ADDRESS_STATE'];
      //echo '</br>' . $row['ADVERTISEMENT_PRICE'] . 'pcm';
